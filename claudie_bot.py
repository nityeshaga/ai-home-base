#!/usr/bin/env python3
"""
Claudie — AI cofounder for Every Consulting.
...
Also supports proactive messaging via CLI:
    python claudie_bot.py --send USER_ID "message"
    python claudie_bot.py --channel "#general" "message"
    echo '{"result":"..."}' | python claudie_bot.py --send-result USER_ID
"""
...
logger = logging.getLogger("claudie")
...
audit_logger = logging.getLogger("claudie.audit")
...
def handle_mention(event, say):
    """Handle @Claudie mentions in channels."""
    user_id = event.get("user", "")
    if not is_authorized(user_id):
        log_unauthorized(event)
        say(text="I only respond to authorized users.", thread_ts=event.get("ts"))
...
def health():
    return jsonify({"status": "ok", "bot": "claudie"})
...
def main():
    parser = argparse.ArgumentParser(description="Claudie — AI PM Bot")
    parser.add_argument(
    ...
    logger.info(f"Claudie starting on port {PORT}")
